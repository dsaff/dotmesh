 ...
 * 
 * Modified for dotmesh by David Saff
 * 
 * @author npryce
 */
public class ClassImposteriser implements Imposteriser {
	...
	private boolean toStringMethodIsFinal(Class<?> type) {
		try {
			...
			throw new IllegalStateException(
					"not allowed to reflect on toString method", e);
		} catch (NoSuchMethodException e) {
			throw new Error("no public toString method found", e);
		}
	}

	private void setConstructorsAccessible(Class<?> mockedType,
			boolean accessible) {
	...
	private <T> Class<?> createProxyClass(Class<?> possibleMockedType,
			Class<?>... ancilliaryTypes) {
		Class<?> mockedType = possibleMockedType == Object.class ? ClassWithSuperclassToWorkAroundCglibBug.class
				: possibleMockedType;
		...
		enhancer.setClassLoader(SearchingClassLoader.combineLoadersOf(
		...
		if (mockedType.getSigners() != null) {
			enhancer
					.setNamingPolicy(NAMING_POLICY_THAT_ALLOWS_IMPOSTERISATION_OF_CLASSES_IN_SIGNED_PACKAGES);
		}
		...
		try {
			...
			// structures.
			throw new IllegalArgumentException("could not imposterise "
					+ mockedType, e);
		}
	}
	...
	private Object createProxy(Class<?> proxyClass, final Invokable mockObject) {
		...
		proxy.setCallbacks(new Callback[] { new InvocationHandler() {
			public Object invoke(Object receiver, Method method, Object[] args)
					throws Throwable {
				return mockObject
						.invoke(new Invocation(receiver, method, args));
			}
		}, NoOp.INSTANCE });
		return proxy;
	}
